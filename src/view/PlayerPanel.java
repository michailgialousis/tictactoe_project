...
public /**/class PlayerPanel extends GamePanel {
    ...
    public PlayerPanel(GameController c, int pos) {
        ...
        selectPlayerBtn.addActionListener((e) -> {
            changePlayer();
        });
    ...
    public void changePlayer() {
        String[] allPlayers = gc.getModel().getPlayerRoster().findPlayerNames().toArray(new String[0]);
        ...
        if (selPlayer != null) {
            Player opponent = gc.getModel().getGamePlayers()[pos == 0 ? 1 : 0];
            String opponentName = "";
            if (opponent != null) opponentName = opponent.getName();
            if (selPlayer.equals(opponentName)) {
            ...
            gc.selectPlayer(gc.getModel().getPlayerRoster().findPlayer(selPlayer), pos);
